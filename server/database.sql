...
-- perference in request 
ALTER TABLE REQUEST ADD COLUMN PREFERENCE_ID INT REFERENCES PREFERENCE(PREFERENCE_ID) ON DELETE SET NULL;
ALTER TABLE REQUEST ADD COLUMN request_token TEXT;

--for all the status
CREATE TYPE req AS ENUM ('PENDING', 'APPROVED', 'REJECTED');
INSERT INTO REQUEST_S (STATUS)
VALUES ('PENDING'), ('APPROVED'), ('REJECTED');
ALTER TYPE req ADD VALUE 'REQUEST_SENT';
INSERT INTO REQUEST_S (STATUS)
VALUES ('REQUEST_SENT');
ALTER TYPE req ADD VALUE 'AVAILABLE';
ALTER TYPE req ADD VALUE 'REJECT DRIVER';
ALTER TYPE req ADD VALUE 'ACCEPT DRIVER';

INSERT INTO REQUEST_S (STATUS)
VALUES ('AVAILABLE'), ('ACCEPT DRIVER'), ('REJECT DRIVER');

-- request sending 
-- MANNUALLY DELETE THE REQUST STATUS FROM RIDE
CREATE TABLE REQUEST_SENT(
   REQUESTSENT_ID SERIAL PRIMARY KEY,
   MEMBER_ID INT NOT NULL,
   RIDE_ID INT NOT NULL,  
   CREATED_ON TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
   STATUS req NOT NULL DEFAULT 'PENDING',
   FOREIGN KEY (MEMBER_ID) REFERENCES MEMBER (MEMBER_ID) ON DELETE CASCADE,
   FOREIGN KEY (RIDE_ID) REFERENCES RIDE (RIDE_ID) ON DELETE CASCADE
);

-- for token
ALTER TABLE REQUEST_SENT ADD COLUMN requestSent_token TEXT;
